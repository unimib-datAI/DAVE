...
export default (app) => {
    ...
    route.get(
        ...
        asyncRoute(async (req, res) => {
            ...
            const documentsPage = await DocumentController.findAll(
                q,
                limit,
                page,
            );
        ...
        for (const annset of document.annotation_sets) {
            ...
            if (annset.name.startsWith("entities")) {
                ...
                for (const annot of annset.annotations) {
                    ...
                    if (
                        typeof annot.id === "string" ||
                        annot.id instanceof String
                    ) {
    ...
    route.post(
        ...
        asyncRoute(async (req, res, next) => {
            ...
            document.features.clusters[annotationSet] =
                document.features.clusters[annotationSet].filter(
                    (cluster) => cluster.id !== sourceCluster,
                );
        ...
        asyncRoute(async (req, res, next) => {
            ...
            await Promise.all(
                Object.values(req.body.annotation_sets).map(async (set) => {
                    ...
                    const annSet =
                        await AnnotationSetController.insertOne(newAnnSet);
            ...
            if (elasticIndex) {
                const elasticUrl =
                    process.env.QAVECTORIZER_ADDRESS ||
                    "http://qavectorizer:7863";
            ...
            if (elasticIndex) {
                ...
                console.log("Document data:", {
                    ...
                    annotation_sets: req.body.annotation_sets
                        ...
                        : [],
                    has_entities:
                        req.body.annotation_sets?.entities_?.annotations
                            ?.length || 0,
                });
                try {
                    ...
                    console.error(
                        "Error posting to Elasticsearch:",
                        error.message,
                    );
                    if (error.response) {
                        console.error(
