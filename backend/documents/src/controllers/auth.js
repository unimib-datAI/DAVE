...
export const AuthController = {
    ...
    async verifyCredentials(email, password) {
        ...
        if (!user) {
            console.log(
                `AuthController.verifyCredentials: no user for ${email}`,
            );
            return null;
        }
        ...
        console.log(
            `AuthController.verifyCredentials: verified ${user.userId}`,
        );
        return user;
    },
    async login(email, password) {
        ...
        if (!user) {
            console.log(
                `AuthController.login: invalid credentials for ${email}`,
            );
    ...
    async refresh(refreshToken) {
        ...
        if (!dbToken) {
            console.log(
                "AuthController.refresh: refresh token not found in DB",
            );
            throw new HTTPError({
    ...
    async meFromJwt(token) {
        ...
        try {
            ...
            console.log(
                `AuthController.meFromJwt: success userId=${payload.sub}`,
            );
            return user;
        } catch (err) {
            console.log(
