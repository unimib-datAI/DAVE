...
const ItemContainer = styled.button<{ active?: boolean; paddingLeft?: number }>(
  ({ active, paddingLeft = 14 }) => ({
    border: 'none',
    outline: 'none',
...
const SidebarAddAnnotation = () => {
  ...
  const { bindings: bindingsAddType, setVisible: setAddTypeVisible } =
    useModal();
  const {
  ...
  return (
    <>
      ...
      <ConfirmationDialog
        {...bindingsDeleteNode}
        {...propsDeleteNode}
        content={t('deleteTypeConfirmation')}
      />
    </>
  );
};
