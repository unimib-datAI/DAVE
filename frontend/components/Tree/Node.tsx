      ...
      <ColoredSquared color={item.color}>
        {hasChildren && (
          <NumberSquare color={item.color}>
            <strong style={{ fontSize: '11px' }}>{nTotalSubChildren}</strong>
          </NumberSquare>
        )}
      </ColoredSquared>
      <NodeTextContainer>
...
function Node({ item, toggle, hasChildren, level, ...props }: NodeProps) {
  ...
  const nTotalSubChildren = useMemo(
    () => (hasChildren ? countChildren(item) : 0),
    [item, hasChildren]
  );
  ...
  return (
    <Container selected={selected} onClick={handleClick}>
      <InnerContainer level={level} hasChildren={hasChildren} {...props}>
