// Ported from backend/documents/src/models/user.js
...
export interface IUser extends Document {
  ...
  createdAt?: Date;
  updatedAt?: Date;
  id: string; // virtual, mirrors userId

  setPassword(plainPassword: string): Promise<string>;
  validatePassword(plainPassword: string): Promise<boolean>;
}

export interface IUserModel extends Model<IUser> {
  ensureDefaultAdmin(): Promise<IUser | null>;
}
 ...
 */
(UserSchema.statics as any).ensureDefaultAdmin = async function (
  this: IUserModel
): Promise<IUser | null> {
  const User = this;
...
export const UserModel: IUserModel =
  ...
  (mongoose.model<IUser, IUserModel>('User', UserSchema, 'users') as IUserModel);
