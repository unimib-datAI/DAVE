// Ported from backend/documents/src/services/keycloak.js
//
// NOTE: admin-client operations here (createUser, deleteUser, setUserRealmRoles,
// updateUser) act on real Keycloak accounts and have not been exercised
// against a live Keycloak instance as part of this port - verify before
...
class KeycloakService {
  ...
  private lastAuth: number | null = null;

  async getAdminClient() {
    // Authenticate if not authenticated or token expired (refresh every 50 seconds)
    if (!this.adminClient || !this.lastAuth || Date.now() - this.lastAuth > 50000) {
      try {
        ...
        console.error('Failed to authenticate with Keycloak admin:', error.message);
        throw error;
      }
    }
  ...
  async getUserById(userId: string) {
    try {
      ...
      console.error(`Error fetching user ${userId} from Keycloak:`, error.message);
      return null;
    }
  }
  ...
  async updateUser(
    ...
    try {
      ...
      console.error(`Error updating user ${userId} in Keycloak:`, error.message);
  ...
  async getUserRealmRoles(userId: string): Promise<string[]> {
    try {
      ...
      return roles.map((r: any) => r.name).filter((n: string) => MANAGED_ROLES.includes(n));
    } catch (error: any) {
  ...
  async setUserRealmRoles(userId: string, roleNames: string[]) {
    try {
      const client = await this.getAdminClient();
      client.setConfig({ realmName: TARGET_REALM });
      ...
      const rolesToRemove = currentRoles.filter(
        (r: any) => MANAGED_ROLES.includes(r.name) && !roleNames.includes(r.name)
      );
      const currentNames = currentRoles.map((r: any) => r.name);
      const rolesToAdd = allRoles.filter(
        (r: any) => roleNames.includes(r.name) && !currentNames.includes(r.name)
      );

      if (rolesToRemove.length > 0) {
        await client.users.delRealmRoleMappings({ id: userId, roles: rolesToRemove });
      }
      if (rolesToAdd.length > 0) {
        await client.users.addRealmRoleMappings({ id: userId, roles: rolesToAdd });
      }
  ...
  async deleteUser(userId: string) {
    try {
      ...
      console.error(`Error deleting user ${userId} from Keycloak:`, error.message);
