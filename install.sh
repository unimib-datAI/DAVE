...
info ""
info "This runs inside a container, so 'localhost' means the container"
info "itself, not your machine. If the LLM runs on this host, use"
info "http://host.docker.internal:<port>/v1 instead (already wired up in"
info "both compose files). If it's another container on dave_net, use its"
info "service name, e.g. http://text-generation:8000/v1."
API_LLM="${API_LLM:-}"
ask API_LLM "  LLM API base URL (e.g. http://host.docker.internal:8001/v1)" "$API_LLM"
